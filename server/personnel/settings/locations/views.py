"""
Views for Locations model CRUD operations.
"""
...
class LocationsListView(generics.ListAPIView):
    queryset = Locations.objects.filter(is_deleted=False)
    serializer_class = LocationsSerializer
    permission_classes = [permissions.IsAuthenticated]
    @handle_exceptions
    @require_permissions("view_locations")
...
class LocationsCreateView(generics.CreateAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]
    @handle_exceptions
    @require_permissions("add_locations")
    def create(self, request, *args, **kwargs):
        ...
        return APIResponse.created("Location created successfully")
...
class LocationsUpdateView(generics.UpdateAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated]
    @handle_exceptions
    @require_permissions("change_locations")
...
class LocationsDeleteView(generics.DestroyAPIView):
    queryset = Locations.objects.filter(is_deleted=False)
    lookup_field = "id"
    permission_classes = [permissions.IsAuthenticated]
    @handle_exceptions
    @require_permissions("delete_locations")
