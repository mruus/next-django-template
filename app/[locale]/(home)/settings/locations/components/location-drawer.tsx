...
import type { Location } from "@/actions/personnel/settings/locations";
...
type LocationDrawerMode = "create" | "edit";
...
export default function LocationDrawer({
  ...
  const updateMutation = useMutation<void, Error, { id: string } & FormValues>({
    mutationFn: async (payload) => {
      const res = await updateLocationAction(payload.id, {
        name: payload.name,
        label: payload.label,
      });
      if (!res.success) throw new Error(res.error || t("toasts.error"));
    ...
    <Drawer direction={direction} open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="sm:max-w-sm overflow-y-auto">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-5 flex flex-col gap-5">
          <Field>
            <FieldLabel htmlFor="location-name">{commonT("name")}</FieldLabel>
            <FieldContent>
              <Input id="location-name" className="w-full" {...register("name")} />
