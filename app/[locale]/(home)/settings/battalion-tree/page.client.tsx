...
import {
  type BattalionTree,
  deleteBattalionTreeAction,
  listBattalionTreesAction,
} from "@/actions/personnel/settings/battalionTree";
import {
  listLabelsAction,
  type Label,
} from "@/actions/personnel/settings/labels";
import BattalionTreeDrawer from "./components/battalion-tree-drawer";

export default function BattalionTreePage() {
  ...
  const [selectedNode, setSelectedNode] = useState<BattalionTree | null>(null);
  const [createParentId, setCreateParentId] = useState<string | null>(null);

  const [treeSelectedId, setTreeSelectedId] = useState<string | null>(null);

  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false);
  const [nodeToDelete, setNodeToDelete] = useState<BattalionTree | null>(null);

  const { data: battalionTreesData, isLoading: isTreeLoading } = useQuery({
    queryKey: ["battalionTrees"],
    queryFn: () => listBattalionTreesAction(),
  });

  const nodes: BattalionTree[] = battalionTreesData?.data?.message || [];
  ...
  const battalionLabels = useMemo(
    () => labels.filter((l) => l.type === "battalion"),
    [labels],
  );
  ...
  const treeInitialData: ArboristNode[] = useMemo(() => {
    const childrenByParent = new Map<string | null, BattalionTree[]>();
  ...
  const deleteMutation = useMutation<void, Error, string>({
    mutationFn: async (id) => {
      const res = await deleteBattalionTreeAction(id);
